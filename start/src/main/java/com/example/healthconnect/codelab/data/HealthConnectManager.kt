...
class HealthConnectManager(private val context: Context) {
    ...
    suspend fun writeWeightInput(weightInput: Double) {
        ...
        val weightRecord = WeightRecord(
            weight = Mass.kilograms(weightInput), time = time.toInstant(),
    ...
    suspend fun writeWeightInput2(weightInput: Double) {
        val time = ZonedDateTime.now().withNano(0)
        val weightRecord = WeightRecord(
            weight = Mass.kilograms(weightInput), time = time.toInstant(),
            zoneOffset = time.offset
        ...
        try {
            ...
            Toast.makeText(context, "Sucessfully insert records", Toast.LENGTH_SHORT).show()
    ...
    suspend fun getChanges(token: String): Flow<ChangesMessage> = flow {
        ...
        do {
            ...
            if (response.changesTokenExpired) {
                throw IOException("Chnages token has expired")
            }
